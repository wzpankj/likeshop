...
class OrderServer
{
    ...
    public function setOrderBaseInfo()
    {
        ...
        if (OrderEnum::ORDER_TYPE_TAKE_AWAY == $this->params['order_type']) {
            $this->dabao_amount = 0.5;
            $this->delivery_amount = $this->shop['delivery_freight'];
        }

        if (OrderEnum::ORDER_TYPE_YOUSELF_TAKE == $this->params['order_type'] && '2'==$this->params['dining_type']) {
            $this->dabao_amount = 0.5;
        }
    ...
    public function checkOrderBaseData()
    {
        ...
        if(OrderEnum::ORDER_TYPE_YOUSELF_TAKE == $this->params['order_type']){
            ...
            if($appoint_time > 1 && ($business_start_time > $appoint_time || $business_end_time < $appoint_time) ){
                throw new Exception('您预约的时间门店已打烊了'.date('Y-m-d H:i',$this->shop['business_end_time']));
            }
    ...
    public function calculateOrder()
    {
        ...
        //总价格（包含运费）
        $this->total_amount = round($this->goods_amount + $this->delivery_amount+ $this->dabao_amount,2);
        return $this;
    }
...
//        }
    ...
    public function getDiscountAmountIntegral(){
        ...
        #最大使用积分额度
        $use_integral=intval($yingfu)*100;

        #用户积分不足直接返回
        if (intval($this->user['integral'])<$use_integral) {
            // echo $this->user['integral']."buzu".$use_integral;
            $this->discount_amount_jifen=intval($this->user['integral'])/100;
            return intval($this->user['integral'])/100;
        }

        #积分足够多直接抵扣所有应付金额
        $this->discount_amount_jifen=intval($yingfu);
   ...
   /**
     * @notes 获取打包费  打包盒外卖 需要 
     * @return mixed
     * @author cjhao
     * @date 2021/9/9 11:25
     */
    public function getDabaoAmount()
    {   
        $amount=0;
        //外卖订单
        if(OrderEnum::ORDER_TYPE_TAKE_AWAY == $this->params['order_type']){
    ...
    public function sumbitOrder()
    {
        ...
        $order->use_integral    = $order->integral_amount*100;              //使用积分额度
