...
class ShopOrderLogic extends Logic
{
    ...
    public static function statistics($get)
    {
        ...
        if (isset($get['shop_info']) && $get['shop_info'] != '') {
            $where[] = ['s.name|s.shop_sn', 'like', '%'.$get['shop_info'].'%'];
        }
        //订单信息
    ...
    public static function lists($get = [])
    {
        ...
        $field = 'o.id,o.user_id,o.order_sn,o.order_status,o.create_time,o.total_num,o.dining_type,o.is_appoint,o.appoint_time,o.take_code,o.consignee,o.mobile,o.delivery_amount,(o.order_amount - o.delivery_amount) as goods_amount,o.order_amount,o.pay_way,o.pay_status,o.pay_time,o.order_type,o.refund_amount,o.surplus_refund_amount,o.refund_status,o.refund_time,s.shop_sn,s.name as shop_name';
    ...
    public static function exportFile($get)
    {
        ...
        $lists = Order::alias('o')
            ->field('o.id,o.user_id,o.order_sn,o.order_status,o.create_time,o.total_num,o.dining_type,o.is_appoint,o.appoint_time,o.take_code,o.consignee,o.mobile,o.delivery_amount,(o.order_amount - o.delivery_amount) as goods_amount,o.order_amount,o.pay_way,o.pay_status,o.pay_time,o.order_type,s.shop_sn,s.name as shop_name')
            ->join('user u', 'u.id = o.user_id')
            ->join('order_goods og', 'og.order_id = o.id')
            ->join('shop s', 's.id = o.shop_id')
            ->with(['order_goods' => function($query){
        ...
        $exportTitle = ['ID', '门店信息', '订单编号', '订单状态', '下单时间', '会员编号', '会员昵称', '商品信息', '商品数量', '就餐方式', '取餐时间', '联系人', '联系方式', '配送费用', '商品金额', '应付金额',
            '支付方式', '支付状态', '支付时间'];
        $exportExt = 'xls';
        $exportData = [];
        foreach($lists as $item) {
            ...
            $exportData[] = [$item['id'],$item['shop_sn'].'-'.$item['shop_name'], $item['order_sn'], $item['order_status_text'], $item['create_time'], $item['user']['sn'], $item['user']['nickname'], $goods_info, $item['total_num'], $item['dining_type_text'], $item['appoint_time'], $item['consignee'], $item['mobile'], $item['delivery_amount'], $item['goods_amount'], $item['order_amount'], $item['pay_way_text'], $item['pay_status_text'], $item['pay_time']];
        }
