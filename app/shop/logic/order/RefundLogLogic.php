...
class RefundLogLogic extends Logic
{
    ...
    public static function lists($get = [], $shop_id)
    {
        ...
        $field = 'o.id,or.id as order_refund_id,or.refund_sn,or.refund_status,or.create_time as refund_time,o.order_sn,o.create_time as order_time,or.refund_type,or.refund_way,or.refund_amount,or.arrival_time,or.transaction_id';
    ...
    public static function exportFile($get,$shop_id)
    {
        ...
        $lists = Order::alias('o')
            ->field('or.id,or.refund_sn,or.refund_status,or.create_time as refund_time,o.order_sn,o.create_time as order_time,or.refund_type,or.refund_way,or.refund_amount,or.arrival_time,or.transaction_id')
            ->join('order_refund or', 'o.id = or.order_id')
            ->join('order_goods og', 'og.order_id = o.id')
            ->join('goods g', 'g.id = og.goods_id')
            ->with(['order_goods' => function($query){
        ...
        $exportTitle = ['ID', '退款编号', '退款状态', '退款时间', '订单编号', '下单时间', '商品信息', '退款类型', '退款路径', '退款金额', '到账时间', '退款流水账号'];
        $exportExt = 'xls';
        $exportData = [];
        foreach($lists as $item) {
            ...
            $exportData[] = [$item['id'], $item['refund_sn'], $item['refund_status_desc'], $item['refund_time'], $item['order_sn'], $item['order_time'], $goods_info, $item['refund_type_desc'], $item['refund_way_desc'], $item['refund_amount'], $item['arrival_time'], $item['transaction_id']];
        }
