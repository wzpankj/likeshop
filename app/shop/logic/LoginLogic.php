...
{
    public static function adminSso($params)
    {
        ...
        $shop_code = Cache::get('shop_id_'.$shop_id);

        if ($code === $shop_code){
            Cache::delete('shop_id_'.$shop_id);
            $shop_admin = new ShopAdmin();

            $admin_info = $shop_admin->alias('SA')
                ->join('shop S', 'S.id = SA.shop_id')
                ->field('S.id,SA.account,SA.name,SA.role_id,SA.shop_id,S.name as shop_name')
                ->where(['SA.shop_id' => $shop_id, 'SA.root'=>1,'SA.del' => 0])
                ->findOrEmpty()->toArray();
            if(empty($admin_info)){
                return false;
            }
