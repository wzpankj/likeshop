...
//            }
    ...
    //生成zhuohao图
    public function makeZhuohao($params=[])
    {
        try {
            $save_dir = 'uploads/qr_code/user_share/';
            $background_img = ROOT_PATH .'/images/zgbg.png';
            !file_exists($save_dir) && mkdir($save_dir, 0777, true);

            $save_key = 'shopid'.$params['shop_id'].'zh'.$params['number']. '.png';
            $qr_src =$save_key;
            $poster_url = ROOT_PATH.'/'.$save_dir . $qr_src;
            ...
            $share_background_img = imagecreatefromstring(file_get_contents($background_img));
            //合成头像
            $qr_code_logic->writeImg($share_background_img, 'uploads/qr_code/shop_id=1_zhuohao=021.png', ['w' => 400, 'h' => 400, 'x' => 105, 'y' => 30,],true);

            $qr_code_logic->writeText($share_background_img, '桌号：'.$params['number'], [
                'color' => '#FF2C3C', 'font_face' => ROOT_PATH.'/font/SourceHanSansCN-Regular.otf', 'font_size' => 30, 'x' => 215, 'y' => 550,
            ]);
            imagepng($share_background_img, $poster_url);
