<?php /*a:2:{s:70:"D:\phpstudy_pro\WWW\dcweb\app\shop\view\order\takeout_order\lists.html";i:1676699056;s:52:"D:\phpstudy_pro\WWW\dcweb\app\shop\view\layout1.html";i:1676699056;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <div class="layui-card">
        ...
        <div class="layui-collapse like-layui-collapse" lay-accordion="" style="border:1px dashed #c4c4c4">
            <div class="layui-colla-item">
                ...
                <div class="layui-colla-content layui-show">
                    <p>*外卖订单列表。</p>
                    <p>*订单状态有待付款，准备中，配送中，已完成，已关闭。</p>
                    <p>*待付款订单取消后则为已关闭。待付款订单支付后则为准备中。准备中订单完成后则为配送中。配送中订单收货后则为已完成。</p>
                </div>
            </div>
        </div>
        </div>
        <div class="layui-tab layui-tab-card" lay-filter="tab-all">
            ...
            <ul class="layui-tab-title">
                ...
                <li data-type="3">配送中（<?php echo htmlentities($statistics['wait_take']); ?>）</li>
                ...
                <div class="layui-form-item">
                    <div class="layui-row">
                        ...
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-block">
                                <input type="text" name="user_info" id="user_info" placeholder="请输入用户昵称/编号查询"
                                       autocomplete="off" class="layui-input" style="width: 188px;">
                            </div>
                        </div>
                ...
                <div class="layui-card">
                    <div class="layui-card-body">
                        ...
                        <div style="padding-bottom: 10px;padding-top: 20px;">
                            ...
                            <button id="notice" class="layui-btn layui-btn-sm layuiadmin-btn-goods <?php echo htmlentities($view_theme_color); ?>" data-type="notice">确认配送</button>
                            <button id="confirm" class="layui-btn layui-btn-sm layuiadmin-btn-goods <?php echo htmlentities($view_theme_color); ?>" data-type="confirm">确认收货</button>
                        </div>
                        ...
                        <script type="text/html" id="table-operation" >
                            <div class="table-operate">
                                ...
                                {{#  if(d.order_status == 1){ }}
                                <a class="layui-btn layui-btn-normal layui-btn-sm operation-btn" lay-event="notice">确认配送</a>
                                {{#  } }}
      ...
      layui.config({
        ...
        function getList(type) {
            table.render({
                ...
                , url: '<?php echo url("order.takeout_order/lists"); ?>?type=' + type
                , cols: [[
                    {type: 'checkbox'}
        ...
        form.on('submit(export)', function(data){
          ...
          var field = data.field;
          $.ajax({
              url: '<?php echo url("order.takeout_order/exportFile"); ?>?type='+ type,
              type: 'get',
              data: field,
              dataType: 'json',
              error: function() {
        ...
        var active = {
            ...
            cancel: function(){
              ...
              // 提交数据
              like.ajax({
                  url:'<?php echo url("order.takeout_order/cancel"); ?>',
                  data:{"order_ids":ids},
                  type:"post",
                  success:function(res) {
                      if(res.code === 1) {
            ...
            confirm: function(){
                ...
                // 提交数据
                like.ajax({
                    url:'<?php echo url("order.takeout_order/confirm"); ?>',
                    data:{"order_ids":ids},
                    type:"post",
                    success:function(res) {
                        if(res.code === 1) {
            ...
            notice: function(){
                ...
                // 提交数据
                like.ajax({
                    url:'<?php echo url("order.takeout_order/notice"); ?>',
                    data:{"order_ids":ids},
                    type:"post",
                    success:function(res) {
                        if(res.code === 1) {
        ...
        table.on('tool(like-table-lists)', function (obj) {
            ...
            if(obj.event === 'remarks'){
                like.ajax({
                    url: '<?php echo url("order.takeout_order/remarks"); ?>'
                    , data: {'id': id}
                    , type: 'get'
                    , success: function (res) {
                        if (res.code === 1) {
            ...
            if(obj.event ==='print'){
                layer.confirm('确定要打印订单吗？', function(index){
                    like.ajax({
                        url:'<?php echo url("order.takeout_order/print"); ?>',
                        data:{id:id},
                        type:"post",
                        success:function(res)
                        {
            ...
            if(obj.event === 'detail'){
                layer.open({
                    type: 2
                    ,title: '订单详情'
                    ,content: '<?php echo url("order.takeout_order/detail"); ?>?id='+id
                    ,area: ['90%', '90%']
            ...
            if(obj.event === 'refund'){
                layer.open({
                    type: 2
                    , title: '订单退款'
                    , content: '<?php echo url("order.takeout_order/refund"); ?>?id='+id
                    , area: ['90%', '90%']
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(' + submitID + ')', function (data) {
                            ...
                            like.ajax({
                                url: '<?php echo url("order.takeout_order/refund"); ?>',
                                data: field,
                                type: "post",
                                success: function (res) {
                                    if (res.code == 1) {
        ...
        function updateTabNumber(field) {
          like.ajax({
              url: '<?php echo url("order.takeout_order/totalCount"); ?>',
              data: field,
              type: "GET",
              success: function (res) {
                  if (res.code === 1) {
                      ...
                      $(".layui-tab-title li[data-type=3]").html("配送中(" + res.data.wait_take + ")");
