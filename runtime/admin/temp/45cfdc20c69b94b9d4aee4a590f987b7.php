<?php /*a:2:{s:63:"D:\phpstudy_pro\WWW\dcweb\app\admin\view\goods\goods\lists.html";i:1682255148;s:53:"D:\phpstudy_pro\WWW\dcweb\app\admin\view\layout1.html";i:1676699056;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <div class="layui-card">
        ...
        <div class="layui-tab layui-tab-card" lay-filter="like-tabs">
            <ul class="layui-tab-title">
                ...
                <li data-type='2' >总部可售(<?php echo htmlentities($statistics['upper']); ?>)</li>
                <li data-type='3' >总部停售(<?php echo htmlentities($statistics['lower']); ?>)</li>
            </ul>

            <div class="layui-tab-content" style="padding: 0 15px;">
                <div style="padding-bottom: 10px;padding-top: 20px;" class="add">
                    <button class="layui-btn layui-btn-sm layuiadmin-btn-goods <?php echo htmlentities($view_theme_color); ?>" data-type="add">新建商品</button>
                ...
                <script type="text/html" id="table-operation">
                    ...
                    {{# } }}
                    <a class="layui-btn layui-btn-danger layui-btn-sm" lay-event="del_open">删除</a>
                </script>
            </div>
        </div>
    </div>
...
<script>
    layui.use(['table', 'form', 'element'], function(){
        ...
        function getList(type) {
            like.tableLists('#like-table-lists', '<?php echo url("goods.goods/lists"); ?>?type='+type, [
                {type: 'checkbox'}
                ...
                ,{field: 'sales_actual',width: 80,title: '销量', align: 'center'}
                ,{field: 'status_desc',width: 150,title: '总部商品状态', align: 'center'}
                ,{field: 'sort',width: 80, title:'排序', align: 'center'}
                ,{field: 'create_time', width: 220,  title:'创建日期', align: 'center'}
            ...
            like.ajax({
                ...
                success: function (res) {
                    if (res.code === 1) {
                        ...
                        $(".layui-tab-title li[data-type=2]").html("总部可售(" + res.data.upper + ")");
                        $(".layui-tab-title li[data-type=3]").html("总部停售(" + res.data.lower + ")");
                    }
                }
            });
        }
        ...
        var active = {
            ...
            lower: function(){
                ...
                // 提交数据
                like.ajax({
                    url:'<?php echo url("goods.goods/changeStatus"); ?>',
                    data:{"goods_ids":ids,"status":0},
                    type:"post",
                    success:function(res) {
                        if(res.code === 1) {
        ...
        form.on('submit(export-file)', function(data){
            ...
            var field = data.field;
            $.ajax({
                url: '<?php echo url("goods.goods/exportFile"); ?>?type='+ type,
                type: 'get',
                data: field,
                dataType: 'json',
                error: function() {
