<?php /*a:2:{s:61:"D:\phpstudy_pro\WWW\dcweb\app\admin\view\shop\auth\lists.html";i:1676699056;s:53:"D:\phpstudy_pro\WWW\dcweb\app\admin\view\layout1.html";i:1676699056;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    layui.config({
        ...
        var insTb = treeTable.render({
            ...
            reqData: function(data, callback) {
                ...
                like.ajax({
                    url:'<?php echo url("shop.auth/lists"); ?>',
                    type:'get',
        ...
        treeTable.on('tool(menu-lists)', function(obj) {
            ...
            if(obj.event === 'del') {
                layer.confirm('确定删除此菜单及其子菜单？', function(index){
                    like.ajax({
                        url:'<?php echo url("shop.auth/del"); ?>',
                        data: {ids:[menu_id]},
                        type:"post",
                        success:function(res)
                        {
            ...
            if(obj.event === 'edit') {
                layer.open({
                    type: 2
                    ,title: '编辑菜单'
                    ,content: '<?php echo url("shop.auth/edit"); ?>?id='+menu_id
                    ,area: ['90%', '90%']
                    ,btn: ['确定', '取消']
                    ,yes: function(index, layero){
                        ...
                        iframeWindow.layui.form.on('submit(menu-submit)', function(data){
                            ...
                            like.ajax({
                                url:'<?php echo url("shop.auth/edit"); ?>',
                                data:field,
                                type:"post",
                                success:function(res)
                                {
        ...
        form.on('switch(switch-disable)',function (obj) {
            ...
            like.ajax({
                url:'<?php echo url("shop.auth/status"); ?>',
                data:data,
                type:"post",
                success:function (res) {
                    if(res.code == 1) {
        ...
        $('#add').click(function(){
            layer.open({
                type: 2
                ,title: '添加菜单'
                ,content: '<?php echo url("shop.auth/add"); ?>'
                ,area: ['90%', '90%']
                ,btn: ['确定', '取消']
                ,yes: function(index, layero){
                    var iframeWindow = window['layui-layer-iframe'+ index]
                    ...
                    iframeWindow.layui.form.on('submit(menu-submit)', function(data){
                        var field = data.field;
                        like.ajax({
                            url:'<?php echo url("shop.auth/add"); ?>',
                            data:field,
                            type:"post",
                            success:function(res)
                            {
